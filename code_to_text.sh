...
find . -type f ! -name requirements.txt ! -path '*/node_modules/*' ! -path '*/dist/*' ! -path '*/build/*' ! -path '*/venv/*' ! -path '*/.venv/*' ! -name "*.lock" ! -name "*.sh" ! -name '*.json' ! -path '*/.*' ! -path './.*' ! -name "*.md" ! -name "*.svg" ! -path "./${output_file}" | while read file; do
